...
(defn make-address [address]
  ...
  (defn make-phone [phone]
   (select-keys phone [:type :number])
  )
...
(defn maybe-set-phone [contact]
  (if (:phone contact)
    (update contact :phone make-phone)
    contact))
  ...
  (let [
    ...
    [:div {:id "contact-form" :class "contact-form"}
     ...
     [:fieldset
      ...
      [:fieldset
      [:legend "Phone"]
      (form-field "input-type" (:type phone) "Type")
      (form-field "input-number" (:number phone) "Phone")      
      ]
    ]
  )
)
...
(defn get-contact-form-data []
  {:first-name (get-field-value "input-first-name")
   ...
   :phone {  
             :type (get-field-value "input-type")
             :number (get-field-value "input-number")             
          }
  }
)
